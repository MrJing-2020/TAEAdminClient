...
(function ($) {
    ...
    var flowDetailTop = 'api/Admin/FlowManager/FlowDetailTop';
    var flowDetailDel = 'api/Admin/FlowManager/FlowDetailDel';
            ...
            success: function (DetailData) {
                ...
                for (var key in DetailData.WorkFlowDetail) {
                    ...
                    stepHtml += [
                        ...
                        '<span class="hvr-icon-bob Top" onclick="initFlowDetailId(this)" id=' + DetailData.WorkFlowDetail[key].Id + '></span>',
                        '<span class="hvr-icon-sink-away Del" onclick="initFlowDetailId(this)" id=' + DetailData.WorkFlowDetail[key].Id + '></span>',
    ...
    //填充步骤弹窗中部门下拉框数据
    var StepDepSelectInit = function (comId, callback) {
        RequestByAjax({
            url: getDepSelectUrl,
            type: 'GET',
            data: { id: comId },
        ...
        detail: function () {
            ...
            RequestByAjax({
                ...
                success: function (response) {
                    ...
                    $('#StepValue').val(response.Step);
                    StepDepSelectInit(response.CompanyId, function () {
                        $('#StepDep').val(response.DepartmentId);
                    })

                    userSelectInit(response.DepartmentId, function () {
    ...
    //})
    $(document).on('change', '#StepDep', function (e) {
        $("#DefualtAuditUserId").empty();
        userSelectInit($(this).val());
    });
    $(document).on('click', '#flowDetailAdd', function (e) {
        $('#FlowDetialId').val("");
        $("#detailEditModalForm").find("input").val("");
        $("#bz").css("display", "block");
        StepDepSelectInit($("#flowCompanyId").val());
        stepData()
    });
    $("#btnFl").click(function () {
        $('#BZ').css("display", "none");
        $("#DefualtAuditUserId").val("");
    })
    //弹框数据提交
